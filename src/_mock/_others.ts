...
export const _notifications = [...Array(9)].map((_, index) => ({
  ...
  title:
    (index === 0 && `<p><strong>15 funcionários</strong> responderam a pesquisa de sentimentos hoje</p>`) ||
    (index === 1 &&
      `<p><strong>Maria Silva</strong> solicitou atendimento psicológico em <strong><a href='#'>Bem-estar Mental</a></strong></p>`) ||
    (index === 2 &&
      `<p>Taxa de engajamento atingiu <strong>85%</strong> na pesquisa desta semana</p>`) ||
    (index === 3 &&
      `<p><strong>João Santos</strong> completou sessão de <strong><a href='#'>Acompanhamento Psicológico<a/></strong></p>`) ||
    (index === 4 &&
      `<p>Relatório mensal de <strong>bem-estar dos funcionários</strong> disponível</p>`) ||
    (index === 5 && `<p>Nova pesquisa de clima organizacional foi publicada</p>`) ||
    (index === 6 && `<p><strong>Alerta:</strong> 3 funcionários com nível de estresse elevado</p>`) ||
    (index === 7 && `<p>Você tem <strong>5 mensagens não lidas</strong> de funcionários</p>`) ||
    (index === 8 && `<p><strong>42 funcionários</strong> participaram de atividades de bem-estar esta semana</p>`) ||
    '',
}));
