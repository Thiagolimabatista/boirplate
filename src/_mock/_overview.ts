...
export const _appInstalled = ['RH', 'TI', 'Financeiro', 'Operações'].map(
  (departamento, index) => ({
    id: _mock.id(index),
    countryName: departamento,
    android: _mock.number.nativeL(index),
    windows: _mock.number.nativeL(index + 1),
    apple: _mock.number.nativeL(index + 2),
    countryCode: ['rh', 'ti', 'fin', 'ops'][index],
  })
);

export const _appAuthors = [
  { name: 'Ambiente de Trabalho', icon: 'solar:buildings-2-bold' },
  { name: 'Liderança e Gestão', icon: 'solar:users-group-two-rounded-bold' },
  { name: 'Benefícios e Remuneração', icon: 'solar:wallet-money-bold' },
].map((topico, index) => ({
  id: _mock.id(index),
  name: topico.name,
  avatarUrl: topico.icon,
  totalFavorites: _mock.number.nativeL(index),
}));

export const _appInvoices = [...Array(5)].map((_, index) => {
  const category = ['Estresse', 'Carga de Trabalho', 'Relacionamento', 'Reconhecimento', 'Infraestrutura'][index];

  const status = ['respondido', 'pendente', 'em análise', 'respondido', 'respondido'][index];

  return {
    id: _mock.id(index),
    invoiceNumber: `USR-${1000 + index}`,
    price: ['Positivo', 'Negativo', 'Neutro', 'Preocupação', 'Positivo'][index],
    category,
    status,
  };
});
...
export const _appFeatured = [
  {
    id: _mock.id(1),
    title: 'Webinar: Gerenciando o Estresse no Trabalho',
    description: 'Participe do nosso próximo evento sobre técnicas de gestão de estresse e bem-estar mental. Data: 15 de Janeiro, 14h',
    coverUrl: _mock.image.cover(1),
  },
  {
    id: _mock.id(2),
    title: 'Programa de Saúde Mental',
    description: 'Novo programa de apoio psicológico disponível para todos os colaboradores. Sessões individuais e em grupo.',
    coverUrl: _mock.image.cover(2),
  },
  {
    id: _mock.id(3),
    title: 'Workshop: Equilíbrio Vida-Trabalho',
    description: 'Aprenda estratégias práticas para melhorar seu equilíbrio entre vida pessoal e profissional. Inscrições abertas.',
