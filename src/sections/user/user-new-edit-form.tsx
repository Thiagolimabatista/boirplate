...
export const NewUserSchema = zod.object({
  avatarUrl: schemaHelper.file({ message: { required_error: 'Foto é obrigatória!' } }),
  ...
  country: schemaHelper.objectOrNull<string | null>({
    message: { required_error: 'País é obrigatório!' },
  }),
  address: zod.string().min(1, { message: 'Endereço é obrigatório!' }),
  company: zod.string().min(1, { message: 'Departamento é obrigatório!' }),
...
export function UserNewEditForm({ currentUser }: Props) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      reset();
      toast.success(currentUser ? 'Atualizado com sucesso!' : 'Criado com sucesso!');
            ...
            <Field.Switch
              ...
              label={
                <>
                  ...
                  <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                    Desativar isso enviará automaticamente um email de verificação
                  </Typography>
                </>
              }
        ...
        <Grid xs={12} md={8}>
          <Card sx={{ p: 3 }}>
            ...
            >
              ...
              <Field.Text name="company" label="Departamento" />
              <Field.Text name="role" label="Cargo" />
            </Box>

            <Stack alignItems="flex-end" sx={{ mt: 3 }}>
              <LoadingButton type="submit" variant="contained" loading={isSubmitting}>
                {!currentUser ? 'Criar funcionário' : 'Salvar alterações'}
              </LoadingButton>
            </Stack>
          </Card>
        </Grid>
