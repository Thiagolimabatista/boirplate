  ...
  const renderTabs = (
    ...
    >
      {[
        ...
        { value: 'subTasks', label: 'Subtarefas' },
        { value: 'comments', label: `Comentários (${task.comments.length})` },
      ].map((tab) => (
  ...
  const renderTabOverview = (
    <Box sx={{ gap: 3, display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center' }}>
        <StyledLabel>Relator</StyledLabel>
        <Avatar alt={task.reporter.name} src={task.reporter.avatarUrl} />
      </Box>

      {/* Assignee */}
      <Box sx={{ display: 'flex' }}>
        <StyledLabel sx={{ height: 40, lineHeight: '40px' }}>Responsável</StyledLabel>
      ...
      {/* Label */}
      <Box sx={{ display: 'flex' }}>
        <StyledLabel sx={{ height: 24, lineHeight: '24px' }}>Etiquetas</StyledLabel>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center' }}>
        ...
        {rangePicker.selected ? (
          ...
          <Tooltip title="Adicionar data de vencimento">
            <IconButton
        ...
        <CustomDateRangePicker
          variant="calendar"
          title="Escolher data de vencimento"
      ...
      {/* Attachments */}
      <Box sx={{ display: 'flex' }}>
        <StyledLabel>Anexos</StyledLabel>
    ...
    <Box sx={{ gap: 3, display: 'flex', flexDirection: 'column' }}>
      <div>
        <Typography variant="body2" sx={{ mb: 1 }}>
          {subtaskCompleted.length} de {SUBTASKS.length}
        </Typography>
